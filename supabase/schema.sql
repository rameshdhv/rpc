...
create sequence if not exists public.rpc_patient_code_seq start 1;
...
create or replace function public.assign_rpc_patient_code()
returns trigger
language plpgsql
as $$
begin
  if new.clinic_patient_code is null or btrim(new.clinic_patient_code) = '' then
    new.clinic_patient_code := 'RPC' || lpad(nextval('public.rpc_patient_code_seq')::text, 4, '0');
  end if;
  return new;
end;
$$;

drop trigger if exists trg_assign_rpc_patient_code on public.patients;
create trigger trg_assign_rpc_patient_code
before insert on public.patients
for each row
execute function public.assign_rpc_patient_code();

grant usage, select on sequence public.rpc_patient_code_seq to authenticated;
